    ...
    <main className="min-h-screen pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="mb-12">
        ...
        <div className="flex flex-wrap gap-2 mt-8">
          <button className="px-5 py-2.5 rounded-xl text-xs font-bold bg-white text-black transition-all hover:opacity-90">
            ALL ITEMS
          </button>
          {categories?.map((cat: any) => (
            <button key={cat.id} className="px-5 py-2.5 rounded-xl text-xs font-bold border border-zinc-800 bg-zinc-900/50 text-zinc-400 hover:text-white hover:border-zinc-700 transition-all uppercase">
              {cat.icon} {cat.name}
            </button>
          ))}
        </div>
